...
if ! [ -z "$x" ]; then
  ...
  #Move the file to an old folder... you could also just delete it at this point
        rm $x
  # Trim log to only the last 50 entries not to make it too long
#       mv log logold
#       tail -n 50 logold > log
...
# M3U8 yo ipns code (noit working)
